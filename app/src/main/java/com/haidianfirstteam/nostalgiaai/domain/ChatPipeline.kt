...
class ChatPipeline(
    ...
    private companion object {
        ...
        private val RENDERING_OUTPUT_SPEC: String = """
            ...
            规则1：数学公式必须用半角 ${'$'}...${'$'} 包裹。例如 ${'$'}x^2+1${'$'}。
            公式请尽量保持在同一行内且前后不要混入其他文字（独占一行渲染最清晰）。
            ...
            规则2：禁止使用全角符号（如 ＄｛｝），只能用半角 ${'$'} { }。

            规则3：Mermaid 必须用 fenced code block：```mermaid ... ```。

            规则4：方程组/分段函数直接用 LaTeX 的 \begin{cases}...\end{cases} 或 \[...\] 格式即可。
        """.trimIndent()
    }
